...
extension CameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
   ...
   func applyAnalogFilter(to image: CIImage) -> CIImage {
      guard let sepia = CIFilter(name: "CISepiaTone"),
            let noise = CIFilter(name: "CINoiseReduction") else { return image }
      
      sepia.setValue(image, forKey: kCIInputImageKey)
      sepia.setValue(0.5, forKey: kCIInputIntensityKey)
      
      noise.setValue(sepia.outputImage, forKey: kCIInputImageKey)
      noise.setValue(0.02, forKey: "inputNoiseLevel")
      
      return noise.outputImage ?? image
   }
}
...
private extension CameraViewController {
   ...
   func updateButtonRotation(with motion: CMDeviceMotion) {
       // Roll: 장치를 좌우로 기울이는 값 (-π ~ π)
       let roll = motion.attitude.roll

       // 버튼을 반대로 회전시켜서 항상 수평으로 유지
       let rotationAngle = -CGFloat(roll)
